...
def get_ee_pose(bot):
    """
    Get end effector pose of WidowX 250s
    """
...
def new_ee_pose(bot, curr, delta):
    """
    Takes in delta of end-effector and executes command
    ...
    epsilon is set to 0.7 for open/close gripper
    """
...
def take_picture(pipeline, rotate=True):
    """
    For taking a picture with Intell Realsense Camera D435
...
if __name__ == "__main__":
    ...
    # Camera Setup
	# We run this command as we found it yielded better images from RealSense D435 at initialization
    ...
    # Put Robot into start position
	# This puts the WidowX250S from sleep position into upright position above an object
    curr = get_ee_pose(bot)
    delta_robot = [0.25, 0, 0.03, 0, 0.875, 0, 1]
    new_ee_pose(bot, curr, delta_robot)
